...
class make_bath(object):
    ...
    def eta_num(self, t, norm='physics'):
        ...
        if self.Temp ==0:
            ...
            re_data = J_func * (1-np.cos(self.wrange * t)) * coth(0.5 * self.wrange/self.Temp)
            im_data = J_func * (np.sin(self.wrange * t) - self.wrange * t)
    ...
    def construct_eta(self, points, dt):
        ...
        for t in self.trange: self.eta.append(self.eta_num(t, norm=self.norm))
    ...
    def check_sample_rate(self, file_name = None):
        ...
        for t in self.trange: self.C.append(self.corr_func(t))
...
class tempo_prop(object):
    ...
    def prop(self, kpoints=1):
        ...
        for k in range(kpoints):
            print('Done time step:', k, ' out of ', kpoints)
            ...
            if self.point< self.dkmax+1 or self.dkmax==0:
                ...
                print('k =', k,' is beyond the memory cut-off.')
